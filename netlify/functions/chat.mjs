...
export async function handler(event) {
  ...
  try {
    const { messages, model = 'ModelsLab/Llama-3.1-8b-Uncensored-Dare', temperature = 0.7, max_tokens = 2048, stream = false } = JSON.parse(event.body);
    ...
    if (result.status === 'error') {
      throw new Error(result.message || 'Chat completion failed');
    }
